...
def hyper_opt(config):
    ...
    assert config.local_rank == -1, 'DDP mode not implemented for --evolve'
    ...
    for _ in range(100):  # generations to evolve
        ...
        # Train mutation
        results = train(hyp.copy(), config, device)

        # Write mutation results
        print_mutation(hyp.copy(), results, yaml_file, config.bucket)
    # Plot results
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/merged.yaml', help='data.yaml path')
